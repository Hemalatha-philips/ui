...
interface QGridSettingsProps<RecordType> {
...
class QGridSettings extends Component<QGridSettingsProps<any>> {
  /**
      ...
      return (
        <React.Fragment key={column.key}>
          <div
            ...
            className="Q-grid-settings__column-container__displayed-columns__list__item"
          >
            <span className="Q-grid-settings__column-container__displayed-columns__list__item__label">
              {column.displayTitle}
              {!this.props.fixedColumns.includes(column.key) ? (
                <i
                  onClick={() => this.onHideColumn(column)}
                  className="fa fa-minus-circle Q-grid-settings__column-container__displayed-columns__list__item__label__icon"
                ></i>
              ) : null}
            </span>
          </div>
    ...
    return hiddenColumns.map((column: Column<any>) => {
      return (
        <div
          ...
          className="Q-grid-settings__column-container__available-columns__list__item"
        >
          <span className="Q-grid-settings__column-container__available-columns__list__item__label">
            {column.displayTitle}
            <i
              onClick={() => this.onShowColumn(column)}
              className="fa fa-plus-circle Q-grid-settings__column-container__available-columns__list__item__label__icon"
            ></i>
          </span>
        </div>
      );
  ...
  render() {
    return (
      <div className="Q-grid-settings">
        <p className="Q-grid-settings__column-heading">
          Select columns to display on the grid. Tap Apply when finished.
        </p>
        <div className="Q-grid-settings__column-container">
          <div className="Q-grid-settings__column-container__available-columns">
            <h4 className="Q-grid-settings__column-container__available-columns__title">
              Available Columns
            </h4>

            {this.props.hiddenColumns && this.props.hiddenColumns.length > 0 ? (
              <div className="Q-grid-settings__column-container__available-columns__list scroll-bar">
                {this.getHiddenColumns()}
              </div>
            ) : null}
          </div>
          <div className="Q-grid-settings__column-container__displayed-columns">
            <h4 className="Q-grid-settings__column-container__displayed-columns__title">
              Displayed Columns
            </h4>

            {this.props.visibleColumns &&
            this.props.visibleColumns.length > 0 ? (
              <div className="Q-grid-settings__column-container__displayed-columns__list scroll-bar">
                {this.getAvailableColumns()}
              </div>
            ) : null}
          </div>
        </div>
        <div className="Q-grid-settings__actions">
          <button
            onClick={(e) => this.onCancelSettings()}
            className="Q-grid-settings__actions__btn Q-grid-settings__actions__btn--cancel"
          >
            Cancel
          </button>
          <button
            className="Q-grid-settings__actions__btn Q-grid-settings__actions__btn--apply"
